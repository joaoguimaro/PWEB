...
function validationName(value) {
  if (value == "" || value.length < 10) {
    alert(
      "⚠️ O campo nome precisa de no minímo 10 caracteres ou está vazio \n  verifique o campo novamente ⚠️"
    );
  }
}
...
function validationEmail(value) {
  if (value.indexOf("@") > -1 == false || value.indexOf(".") > -1 == false) {
    alert(
      "⚠️ Informe um endereço de email válido\n verifique o campo novamente ⚠️"
    );
  }
}
...
function validationCommentary(value) {
  if (value.length < 20) {
    alert(
      "⚠️ O campo comentário precisa de no minímo 20 caracteres\n verifique o campo novamente ⚠️"
    );
  }
}
...
function validationResponse(valueYes, valueNo) {
  if (valueYes.checked == false && valueNo.checked == false) {
    alert("⚠️ selecione Sim ou Não ⚠️");
  } else {
